# run_seam_carver.py
# Wrapper script that runs seam carving without needing TkAgg/Tkinter

import matplotlib
matplotlib.use("Agg")  # Non-GUI backend - must be BEFORE importing pyplot
...
class SeamCarver:
    ...
    def save_comparison(self, path="comparison.png"):
...
def main():
    ...
    if input_img is None:
        print("Error: Could not read data/human.png")
        return
    ...
    if not carver.validate():
        print("Error: Required dimensions are larger than image dimensions.")
        return

    print("Starting seam carving...")
    carver.seam_carving()

    carver.save_comparison("comparison.png")
